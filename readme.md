...
- **Комнаты изоляции (SafeGuard CLI + GUI)**:
  ...
  - **Офлайн-установка из ISO (v0.10.0)**: инсталлятору больше не нужен интернет. При сборке ISO в `/opt/sg-offline` зашивается **полный набор пакетов** (все пакеты установки + зависимости), базы репозиториев и keyring — через `pacman -Sw` со свежим `--dbpath` (ничего не считается установленным → скачивается весь клозур, даже то, что уже есть в live). Инсталлятор проверяет сеть (`geo.mirror.pkgbuild.com`): есть интернет → обычный `pacstrap` (свежие версии); нет → установка из офлайн-набора ISO (`pacman -S` по локальным базам, без сети). Цена: ISO вырастает с ~1.3 ГБ до ~3-4 ГБ.
...
```bash
curl -L -o safegamingos-0.11.0-x86_64.iso \
  https://github.com/met4444m-collab/safegameos/releases/download/v0.11.0/safegamingos-0.11.0-x86_64.iso
```

Вместо `0.11.0` подставь актуальную версию со страницы
https://github.com/met4444m-collab/safegameos/releases (в релизе лежат и ISO, и
слой обновления `safegamingos-files-*.tar.zst` для `sg-update`).
...
sudo ./build_iso.sh          # → out/safegamingos-0.11.0-x86_64.iso
```

Запись на флешку:

```bash
sudo dd bs=4M if=out/safegamingos-0.11.0-x86_64.iso of=/dev/sdX status=progress oflag=sync
```
...
1. Создайте VM с UEFI (или используйте QEMU): загрузите ISO.
   ...
   qemu-system-x86_64 -enable-kvm -m 4096 -cpu host -smp 4 \
     ...
     -cdrom out/safegamingos-0.11.0-x86_64.iso -boot d
   ```
