...
sudo ./build_iso.sh          # → out/safegamingos-0.10.0-x86_64.iso
```

Запись на флешку:

```bash
sudo dd bs=4M if=out/safegamingos-0.10.0-x86_64.iso of=/dev/sdX status=progress oflag=sync
```
...
1. Создайте VM с UEFI (или используйте QEMU): загрузите ISO.
   ...
   qemu-system-x86_64 -enable-kvm -m 4096 -cpu host -smp 4 \
     ...
     -cdrom out/safegamingos-0.10.0-x86_64.iso -boot d
   ```
