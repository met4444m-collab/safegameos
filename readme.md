...
- **Инсталлятор на диск**: `sudo sg-install` (или иконка на рабочем столе) — выбор диска, пользователь, пароль; ставит систему через archinstall и брендирует её: та же загрузочная анимация, обои, swapfile 16 ГБ, SafeGuard, multilib + Wine/Steam/Lutris для Windows-игр.
- **Комнаты изоляции (SafeGuard CLI)**:
  - `sg-room-run NAME -- приложение` — запуск приложения в изолированной комнате (firejail: отдельные namespace, приватный home, `--noroot`, `--caps.drop=all`, `--seccomp`).
...
```bash
sudo sg-setup                                   # настройка базы и сервисов
sudo sg-room-run browser -- firefox             # браузер в комнате
...
- **GUI-центр безопасности** (виджет KDE): комнаты, разрешения, журнал загрузок и блокировок — без терминала.
- **Авто-трекинг загрузок браузера** — интеграция с Firefox (расширение/скрипт), чтобы источник URL записывался автоматически.
- **Ужесточение изоляции**: AppArmor-профили для комнат, cgroup-лимиты памяти/CPU (значения `--mem`/`--cpu` уже хранятся в базе), микро-CPU-планирование.
