...
Обновлено после раунда усиления «harden-4» (см. git-историю).
...
| 16 | **KLauncher (`org.kde.klauncher5`)** — из комнаты с session bus можно дёрнуть `start_service_by_desktop_name` / `exec_blind` → запуск произвольной программы от имени хост-юзера **вне** песочницы | ⚠️ не-игровые комнаты: закрыто (`--dbus-user=none`). Игровые: session bus открыт (нужен OBS/портал) — klauncher достижим. `start_service_by_desktop_name` с аргументами-строками не выполняет shell (аргументы передаются как файлы приложению) — реально опасен `exec_blind`. План: per-room UID/сессия, либо `--dbus-user=filter` (нужен прогон в VM, чтобы не сломать OBS) |
