...
Обновлено после раунда усиления v0.9.3 (per-room UID + AppArmor + авто-трекинг).
...
| 11 | **/dev/shm** | ✅ для изолированных комнат (per-room UID: файлы /dev/shm чужого UID ядром закрыты); общий только у session-комнат (browser/games/streaming) |
