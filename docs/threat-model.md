...
Обновлено после раунда v0.9.7 (аудит blue team: закрыта hosts-инъекция через URL загрузки, коллизия per-room UID, межкомнатный kill по префиксу имени, SQLi-экранирование, `--machine-id` больше не ломает звук игровым комнатам).
